...
class CustomDataset(Dataset):
    ...
    def getInstanceInfo(self, xyz, instance_label, semantic_label):
        pt_offset_label = np.ones((xyz.shape[0], 4), dtype=np.float32) * -100.0     # (N, disx, disy, disz, candidate_gt)
        instance_pointnum = []
        instance_cls = []
        ...
        for i_ in range(instance_num):
            ...
            pt_offset_label[inst_idx_i, 0:3] = xyz_i.mean(0)
            instance_pointnum.append(inst_idx_i[0].size)
            cls_idx = inst_idx_i[0][0]
            instance_cls.append(semantic_label[cls_idx])

            # 对叶片点云中点进行另外求法
            if semantic_label[cls_idx] == 1 or semantic_label[cls_idx] == 2:
                temp = []
                for p in xyz_i:
                    temp.append(np.linalg.norm(np.sum(xyz_i - p, axis=0)))
                temp = np.argsort(np.array(temp))
                pt_offset_label[inst_idx_i, 0:3] = xyz_i[temp[:5]].mean(0)

            pt_offset_label[inst_idx_i, 0:3] = pt_offset_label[inst_idx_i, 0:3] - xyz_i

            gt_dist = np.linalg.norm(pt_offset_label[inst_idx_i, 0:3], axis=-1)
            # gt_min = xyz_i.min(0)
            # gt_max = xyz_i.max(0)
            # gt_r = np.max(gt_max - gt_min)       # 取实例的包围球边长
            gt_r = gt_dist.max()
            Sigma = 25       # 用于调节候选热图，越小越分散
            candidate_gt = np.exp(-Sigma * ((gt_dist**2) / ((gt_r + 1e-4)**2)))

            pt_offset_label[inst_idx_i, 3] = candidate_gt
    ...
    def __getitem__(self, index):
        ...
        # if scan_id in self.ignore_point_id:
        #     del_points_id = self.ignore_point_id[scan_id]
        #     for di in del_points_id:
        #         all_points_id = set(tuple(range(len(data[3]))))
        #         lest_points_id = np.asarray(list(all_points_id - set(tuple(di))))
        #         temp1 = data[0][lest_points_id, :]
        #         temp2 = data[1][lest_points_id, :]
        #         temp3 = data[2][lest_points_id]
        #         temp4 = data[3][lest_points_id]
        #         data = tuple([temp1, temp2, temp3, temp4])
    ...
    def collate_fn(self, batch):
        ...
        for data in batch:
            ...
            instances3D_labels.append((instance_label.clone(), semantic_label.clone()))
        ...
        voxel_coords, v2p_map, p2v_map = voxelization_idx(coords, batch_id)     # np -> nv
        return {
            'scan_ids': scan_ids,
            'coords': coords,
            ...
            'batch_size': batch_id,
            'instances3D_labels': instances3D_labels,
        }
