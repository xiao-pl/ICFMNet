...
def evaluate_candidate_mae(pred_list, gt_list, gt_instance_list, ignore_label=-100, logger=None):
    gt = np.concatenate(gt_list, axis=0)
    pred = np.concatenate(pred_list, axis=0)
    gt_instance = np.concatenate(gt_instance_list, axis=0)
    pos_inds = gt_instance != ignore_label

    # gt_offsets = gt[pos_inds, 0:3]   # (N, 3)
    # gt_dist = np.linalg.norm(gt_offsets, axis=-1)
    # gt_r= np.max(gt[pos_inds, 6:9] - gt[pos_inds, 3:6], axis=-1)    # 取实例的包围球
    # candidate_gt = np.exp(-25*((gt_dist**2)/((gt_r+1e-4)**2)))  # (N)

    candidate_gt = gt[pos_inds]

    pred = pred[pos_inds]
    mae = np.abs(candidate_gt - pred).sum() / pos_inds.sum()
    logger.info(f'Candidate MAE: {mae:.3f}')
    return mae
